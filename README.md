...
- **视频下载**：粘贴链接即可下载，支持多任务并行（1-8 个）、播放列表自动展开、格式/分辨率/HDR/码率选择、HLS/DASH 协议识别、断点续传、完成后打开输出文件夹。
...
1. **首次启动**（如果环境变量中有依赖程序可跳过）：在主界面"设置 → 依赖检测"中点击对应「下载」按钮安装 yt-dlp 与 ffmpeg（会自动放入应用目录下 `.tools/`）。
2. **下载视频**：在下载页粘贴视频链接后点击「添加任务」→ 点击任务行「获取分辨率」→ 选择清晰度后「开始下载」。
3. ~~**下载会员内容**：先在常用浏览器中登录目标网站，再到「设置」的「Cookie 来源」下拉选择该浏览器，之后下载会自动携带登录态。~~ 目前似乎只有Firefox可以获取。
